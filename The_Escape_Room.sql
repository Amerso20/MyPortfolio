...
CREATE TABLE log_duration (
 record_date datetime default now(),
 booking_id INT,
 room_id INT,
 duration int,
 foreign key (booking_id) references booking_record(booking_id),
 foreign key (room_id) references room(room_id)
);
DROP TRIGGER league_table_update_trigger;
CREATE TRIGGER booking_record_update
AFTER UPDATE ON booking_record
FOR EACH ROW INSERT INTO log_duration (booking_id,room_id,duration) VALUES (NEW.booking_id,NEW.room_id,NEW.time_taken_mins);
UPDATE booking_record SET time_taken_mins=99 WHERE booking_id=11031;
